...
def home(request):
    ...
    # Phân trang sản phẩm
    paginator = Paginator(products, 8)  # Giới hạn 10 sản phẩm mỗi trang
...
GEMINI_API_KEY = 'AIzaSyC_bQ6bqre4k0Q5Jt9JOE9iBOaASwMejV0'
genai.configure(api_key=GEMINI_API_KEY)
model = genai.GenerativeModel('gemini-pro')

# Hàm lấy nhận xét và gợi ý sách từ Gemini
def get_gemini_comment_and_book_suggestions(grades):
    prompt = (
        f"Given the following IT subject grades: {grades}, provide a detailed comment in Vietnamese about the performance in each subject, including strengths and areas for improvement. Then suggest one specific book to help improve the performance in the subjects.'"
    )

    print("Prompt sent to Gemini:", prompt)  # In ra prompt gửi đến Gemini
    ...
    try:
        ...
        # In câu trả lời từ Gemini ra terminal
        print("Gemini's response:", response_text)  # In ra câu trả lời

        # Trả về toàn bộ phản hồi như một đoạn văn bản duy nhất
        return response_text
    except Exception as e:
        print(f"Error in Gemini response: {e}")  # In lỗi nếu có
        return "Không thể lấy nhận xét từ Gemini."


# Hàm đệ quy để trích xuất môn học từ block dữ liệu
...
def process_response_to_table(data):
    ...
    if 'content' in data:
        ...
        return filtered_subjects  # Trả về danh sách các môn học đã lọc
    return []

# Hàm tạo bảng HTML từ dữ liệu môn học
def create_table_html(subjects):
    table_html = '<table class="table table-bordered"><thead><tr><th>Tên Môn Học</th><th>Điểm</th><th>Điểm Chữ</th><th>Điểm 4</th></tr></thead><tbody>'
    
    for subject in subjects:
        table_html += f'<tr><td>{subject["Subject Name"]}</td><td>{subject["Subject Mark"]}</td><td>{subject["Char Mark"]}</td><td>{subject["Mark4"]}</td></tr>'
...
def profile_view(request):
    ...
    try:
        ...
        subjects = process_response_to_table(data)
        table_html = create_table_html(subjects)

        grades = ', '.join([f"{subject['Subject Name']}: {subject['Subject Mark']}" for subject in subjects])
        comment_and_books = get_gemini_comment_and_book_suggestions(grades)  # Giả sử hàm này trả về nhận xét

        # Lọc các sản phẩm đã mua và thành công (is_successful=True)
        products = Product.objects.filter(is_successful=True)

        # Tính tổng tiền bỏ ra cho từng tháng
        month_data = defaultdict(int)  # Dữ liệu cho biểu đồ theo tháng
        total_spent = 0  # Biến để lưu tổng tiền bỏ ra
        for product in products:
            month_data['10'] += product.raw_price  # Giả sử tất cả sản phẩm được mua trong tháng 10
            total_spent += product.raw_price  # Cộng tổng tiền

        # Dữ liệu cho biểu đồ: Chi phí theo từng tháng
        months = [str(i) for i in range(1, 13)]  # Thay vì tên tháng, sử dụng số từ 1 đến 12
        amounts = [month_data[str(i)] for i in range(1, 13)]
        ...
        ax.set_title('Biểu đồ chi phí theo tháng')
        ...
        # Mã hóa biểu đồ thành base64
        chart_base64 = base64.b64encode(buffer.getvalue()).decode('utf-8')  # Mã hóa và giải mã lại để có chuỗi base64

        # Trả về biểu đồ dưới dạng hình ảnh và tổng tiền, nhận xét
        return render(request, 'profile.html', {
            'table_html': table_html,
            'comment_and_books': comment_and_books,  # Truyền nhận xét và sách gợi ý
            'products': products,  # Truyền danh sách sản phẩm vào template
            'months': months,
            'amounts': amounts,  # Dữ liệu cho biểu đồ
            'chart': chart_base64,  # Truyền biểu đồ đã mã hóa
            'total_spent': total_spent,  # Truyền tổng tiền vào template
        })
